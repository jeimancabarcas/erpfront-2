...
import type { QueryParams } from '../../../../models/pagination.model';
...
@Component({
  ...
  template: `
    <app-breadcrumb
      [items]="[{ label: 'Abastecimiento', link: '/abastecimiento' }, { label: 'Compras' }]"
    />

    <header
      class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 mt-6"
    >
      <div>
        <h1 class="text-3xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight mb-2">
          Ordenes de Compra
        </h1>
        <p class="text-gray-500 dark:text-gray-400 font-medium">
          Gestiona y realiza seguimiento a tus pedidos a proveedores.
        </p>
      </div>
    ...
    <!-- Barra de Filtros -->
    <div
      class="bg-white dark:bg-gray-900 p-6 rounded-[28px] border border-gray-100 dark:border-gray-800 shadow-sm mb-6"
    >
    ...
    <div
      class="bg-white dark:bg-gray-900 rounded-[28px] shadow-[0_8px_30px_rgb(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-gray-800 overflow-hidden"
    >
      <ui-table
      ...
      >
        ...
        <ng-template uiTableCell="orderNumber" let-item>
          <span
            class="font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/30 px-3 py-1 rounded-lg text-xs tracking-tight border border-indigo-100/50 dark:border-indigo-800/30"
          >
            {{ item.orderNumber }}
          </span>
        </ng-template>
        ...
        <ng-template uiTableCell="supplier" let-item>
          ...
          <div class="text-[10px] text-gray-400 dark:text-gray-500 font-medium tracking-wide">
            {{ item.supplier?.nit }}
          </div>
        </ng-template>
        ...
        <ng-template uiTableCell="date" let-item>
          <span class="text-gray-500 dark:text-gray-400 text-xs font-medium">{{
            item.orderDate | date: 'dd MMM, yyyy'
          }}</span>
        </ng-template>
        ...
        <ng-template uiTableCell="items" let-item>
          <span
            class="bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 px-2 py-1 rounded text-[10px] font-black"
          >
        ...
        <!-- Empty state -->
        <ng-container empty>
          <p class="text-sm text-gray-300 dark:text-gray-600 max-w-xs">
      ...
      <div
        class="flex items-center justify-between px-6 py-4 border-t border-gray-50 dark:border-gray-800"
      >
