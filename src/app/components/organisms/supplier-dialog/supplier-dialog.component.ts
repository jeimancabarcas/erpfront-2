import { Component, inject, signal, input, output, type OnInit } from '@angular/core';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ButtonAtom, TextInputComponent],
  template: `
    @if (loading()) {
      ...
      <div class="flex flex-col items-center gap-2 text-red-500 py-12">
        ...
        <button
          (click)="onClose()"
          class="!rounded-full !px-6 !h-10 !text-sm !font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors mt-4"
        >
          Cerrar
        </button>
      </div>
      ...
      <div class="bg-white dark:bg-gray-900 p-8">
        ...
        <form #supplierForm="ngForm" class="space-y-6">
          ...
          <div class="flex justify-end gap-3 pt-6">
            <ui-button variant="outline" (clicked)="onClose()"> Cancelar </ui-button>
            <ui-button
              variant="primary"
...
export class SupplierDialogOrganism implements OnInit {
  ...
  private dialogRef = inject<MatDialogRef<SupplierDialogOrganism>>(MatDialogRef, {
    optional: true,
  });
